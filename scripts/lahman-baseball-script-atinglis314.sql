...
/* PLAN:
SELECT sum(attendance)/sum(games) AS avg_attendance, team name, park name
FROM homegames
JOIN teams
ON homegame.team = teams.teamid
JOIN parks
ON homegame.park = parks.parkid
WHERE year = 2016 AND sum(games) >= 10
GROUP BY team name, park name
